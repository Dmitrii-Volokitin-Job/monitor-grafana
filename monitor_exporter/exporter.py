...
def try_keycloak_version_from_wellknown(target: dict) -> str:
    ...
    try:
        ...
        logger.debug("Keycloak well-known version probe failed for %s: %s", url, e)
    return "unknown"
...
_VERSION_USER_AGENT = "Monitor-Monitor/2.0-Grafana"


def _fetch_version_json(url: str, timeout: int, strategy_name: str) -> dict | None:
    """Fetch a version JSON payload for one of the extract_version_* strategies.

    Returns the parsed JSON dict on HTTP 200, or None on any failure (non-200,
    bad JSON, network error). Centralises the requests boilerplate that used to
    be copy-pasted across every strategy.
    """
    try:
        resp = requests.get(url, timeout=timeout, verify=False,
                            headers={"User-Agent": _VERSION_USER_AGENT})
        if resp.status_code == 200:
            return resp.json()
    except Exception as e:
        logger.debug("%s version check failed for %s: %s", strategy_name, url, e)
    return None
...
def extract_version_spring_actuator(url: str, timeout: int) -> str:
    ...
    """
    data = _fetch_version_json(url, timeout, "Spring actuator")
    if data is None:
        return "unknown"
...
def extract_version_camunda(url: str, timeout: int) -> str:
    ...
    """
    data = _fetch_version_json(url, timeout, "Camunda")
    if data is None:
        return "unknown"
...
def extract_version_openapi(url: str, timeout: int) -> str:
    ...
    """
    data = _fetch_version_json(url, timeout, "OpenAPI")
    if data is None:
        return "unknown"
    info = data.get("info", {})
...
def extract_version_gateway(url: str, timeout: int) -> str:
    ...
    """
    data = _fetch_version_json(url, timeout, "Gateway")
    if data is None:
        return "unknown"
...
def extract_version_kubernetes(url: str, timeout: int) -> str:
    ...
    """
    data = _fetch_version_json(url, timeout, "Kubernetes")
    if data is None:
        return "unknown"
...
def extract_version_generic_json(url: str, timeout: int) -> str:
    ...
    """
    data = _fetch_version_json(url, timeout, "Monitor")
    if data is None:
        return "unknown"
    return data.get("versionNumber", "") or "unknown"
