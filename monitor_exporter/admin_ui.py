...
_INT_FIELDS = ("expected_status_code", "timeout_seconds", "priority", "db_port")


def _coerce(name: str, raw: str | None):
    """Turn form strings into the value type the column expects.

    Blank input → None (the user left the field empty).
    Numeric int fields raise ValueError on un-parseable input — callers MUST
    handle that to surface a user-friendly form error. Previously the failure
    silently became None, which conflated "left blank" with "typed garbage"
    and let admin-UI submits store rows with missing required values.
    """
    if raw is None or raw == "":
        return None
    if name in _INT_FIELDS:
        return int(raw)  # raises ValueError on bad input — caller handles
    return raw.strip()
...
def _validate(form: dict) -> list[str]:
    ...
    # Reject un-parseable integers with a clear per-field message instead of
    # silently coercing them to NULL — see _coerce() docstring.
    for f in _INT_FIELDS:
        ...
        try:
            ...
            errors.append(f"{f} must be an integer (got {raw!r})")
    return errors
